...
@Component({
  ...
  styleUrls: ['./lista-prueba.component.css']
})
export class ListaPruebaComponent implements OnInit {
  ...
  public dataTestEstudiantes!: any[];
  ...
  */
   ...
   * indexTestPrueba
   */
  public indexTestPrueba() {

    this.cargando = true;
    this.testServices.indexTest(this.idTest)
      .subscribe({
        next: (resp => {
          const { tests } = resp;
          // console.log(resp);

          this.dataTestEstudiantes = tests;
          // console.log(this.dataTestEstudiantes);
    ...
    this.searchTerm$.pipe(
      ...
      .subscribe(texto => {
        ...
        if (texto.length === 0) {
          ...
          this.testServices.searchTest(formData)
            .subscribe(({ tests }) => {

              // this.dataTestEstudiantes
              this.dataTestEstudiantes = tests;
